 ...
 * 
 * Reliable handler for MailChimp subscriptions using the official SDK
 * - Uses @mailchimp/mailchimp_marketing package for guaranteed compatibility
...
/**
 * Handles request for MailChimp subscription using the official SDK
 */
const handler = async (req, res) => {
  ...
  if (req.method !== 'POST') {
    console.log(`[SDK] Error: Method ${req.method} not allowed`);
  ...
  if (!req.body || !req.body.email) {
    console.log('[SDK] Error: Missing email in request');
  ...
  if (!emailRegex.test(email)) {
    console.log('[SDK] Error: Invalid email format');
    return res.status(400).json({ 
      error: 'Bad request',
  ...
  console.log(`[SDK] Processing subscription for: ${email}`);
  ...
  try {
    ...
    if (!apiKey || !listId || !serverPrefix) {
      console.log('[SDK] Error: Missing MailChimp configuration');
    ...
    try {
      ...
      if (checkError.status === 404) {
        console.log('[SDK] Member does not exist, adding to list');
        ...
        console.log('[SDK] Successfully added member to list');
        return res.status(200).json({
          status: 'success',
        ...
        console.error('[SDK] Error checking member:', checkError.message);
        throw checkError;
      }
    }
  } catch (error) {
    console.error('[SDK] MailChimp API error:', error.message || 'Unknown error');
